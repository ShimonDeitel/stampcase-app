import XCTest
@testable import Stampcase

@MainActor
final class StampcaseTests: XCTestCase {
    var store: Store!
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(StampEntry(title: "Test", detail1: "A", detail2: "B", note: "", date: Date()))
        XCTAssertEqual(store.entries.count, before + 1)
    }

    func testDeleteEntryDecreasesCount() {
        store.add(StampEntry(title: "ToDelete", detail1: "A", detail2: "B", note: "", date: Date()))
        let before = store.entries.count
    ...
    func testCannotAddMoreWhenAtLimitAndNotPro() {
        while store.entries.count < Store.freeLimit {
            store.add(StampEntry(title: "Filler", detail1: "A", detail2: "B", note: "", date: Date()))
        }
    ...
    func testUpdateEntryChangesTitle() {
        store.add(StampEntry(title: "Original", detail1: "A", detail2: "B", note: "", date: Date()))
